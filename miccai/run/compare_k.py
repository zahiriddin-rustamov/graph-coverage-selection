...
Usage:
    python -m miccai.run.compare_k --method graph_a2 --k-values 5 10 20 50 100 \
        --dataset organsmnist --embedding trained --ratio 0.02 --size 128

    # With linear probe evaluation
    python -m miccai.run.compare_k --method graph_a2 --k-values 5 10 20 50 \
    ...
    python -m miccai.run.compare_k --method graph_a2 --k-values 5 10 20 \
...
from miccai.run.embeddings import load_or_compute_embeddings, extract_embeddings_with_model
from miccai.run.selection import select
from miccai.run.evaluation import evaluate_linear_probe, evaluate_selection
from miccai.run.results import (
    ...
    if args.linear_probe:
        ...
        if trained_model is not None:
            ...
            test_emb_data = load_or_compute_embeddings(
                ...
                size=args.size, seed=args.seed,
                cache_dir=Path('miccai/cache/embeddings'), verbose=False)
